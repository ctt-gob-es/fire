...
public class MailPasswordRestorationService extends HttpServlet{
	...
	protected void doPost(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
		...
		try {
        	...
        	response.sendRedirect("Login.jsp?" + ServiceParams.PARAM_ERR + "=" + UserMessages.ERR_INCORRET_MAIL.getCode()); //$NON-NLS-1$ //$NON-NLS-2$
        	return;
		}
		...
		if (user == null || user.getMail() == null) {
        	...
        	response.sendRedirect("Login.jsp?" + ServiceParams.PARAM_ERR + "=" + UserMessages.ERR_INCORRET_USER.getCode()); //$NON-NLS-1$ //$NON-NLS-2$
        	return;
		}
		...
		if (!logginPermission) {
			LOGGER.log(Level.WARNING, "El usuario " + user.getId() + " no tiene permiso de acceso. No se le enviara el enlace al correo."); //$NON-NLS-1$ //$NON-NLS-2$
		...
		// Crear URL
		final String url = getRestorationPageUrl(request) + "?" + ServiceParams.PARAM_CODE + "=" + cod; //$NON-NLS-1$ //$NON-NLS-2$
		...
		String subject = null;
		final StringBuilder bodyBase = new StringBuilder();

		try (final InputStream is = MailPasswordAuthoritationService.class.getResourceAsStream("/mail_reset_password.txt"); //$NON-NLS-1$
				final InputStreamReader isr = new InputStreamReader(is,  StandardCharsets.UTF_8);
				final BufferedReader b = new BufferedReader(isr);) {
			String line;
			while ((line = b.readLine()) != null) {
				if (subject == null) {
					...
					bodyBase.append(line).append("<br>"); //$NON-NLS-1$
				}
			}
		}

		// Integrar el nombre de la persona y la URL en el mensaje
		final String body = bodyBase.toString()
				.replace("%%NAME%%", user.getFirstName()) //$NON-NLS-1$
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "No se ha podido enviar el mensaje de restauracion de contrasena", e); //$NON-NLS-1$
			response.sendRedirect("Login.jsp?" + ServiceParams.PARAM_ERR + "=" + UserMessages.ERR_SEND_MAIL.getCode()); //$NON-NLS-1$ //$NON-NLS-2$
	        return;
		}

...
public class ModifyPasswdUserService extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//Obtener el tipo de operación 2-Edición, 3 Cambio de contraseña
		final int op = Integer.parseInt(req.getParameter(PARAM_OP));//$NON-NLS-1$ 
		final String stringOp = op == 3 ? "clave" : "" ; //$NON-NLS-2$	
		...
		try
		{
			...
			if(this.getUser_name()==null || this.getOld_password()==null || 
				...
				LOGGER.log(Level.SEVERE, "No se han proporcionado todos los datos requeridos para cambiar la clave (Nombre de usuario , clave antigüa, Clave nueva y repetir clave nueva)"); //$NON-NLS-1$
				isOk = false;
			}
			...
			//Comprobamos que la clave introducida como antigüa es la que tiene guardada en la base de datos
			else if(usr.getClave()!=null && this.getOld_password()!=null && !"".equals(this.getOld_password())) {
				final MessageDigest md_oldPass = MessageDigest.getInstance(SHA_2);
				byte[] digest_oldPass;//$NON-NLS-1$
				...
				if(!oldPasswd.equals(usr.getClave())) {
					LOGGER.log(Level.SEVERE, "La clave antigüa introducida es erronea."); //$NON-NLS-1$
					isOk = false;
				}
				else {
					...
					try {
						...
						if(op==3) {		
							...
							if(this.getNew_password()!=null && !"".equals(this.getNew_password())) {
							...
							try {
								UsersDAO.updateUserPasswd(this.getIdUser() , this.getUser_name(),clave);//$NON-NLS-1$	
							}
							catch (SQLException e) {
								LOGGER.log(Level.SEVERE, "Error en la modificación de la contraseña del usuario con nombre:" + usr.getNombre_usuario(), e); //$NON-NLS-1$
								isOk = false;
							}
			...
			resp.sendRedirect("./User/UserPage.jsp?error=true&name=" + this.getUser_name() ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
		}
	...
	/**
	 * Obtiene los parámetros pasados
	 * @param req
	 * @throws IOException
	...
	private void getParameters(HttpServletRequest req) throws IOException, ServletException {
		...
		if(req.getParameter(PARAM_NEW_PASSWD)!=null && !"".equals(req.getParameter(PARAM_NEW_PASSWD))) {
			this.setNew_password(req.getParameter(PARAM_NEW_PASSWD));
		}
		if(req.getParameter(PARAM_OLD_PASSWD)!=null && !"".equals(req.getParameter(PARAM_OLD_PASSWD))) {
			this.setOld_password(req.getParameter(PARAM_OLD_PASSWD));
		}
		if(req.getParameter(PARAM_NEW_PASSWD_COPY)!=null && !"".equals(req.getParameter(PARAM_NEW_PASSWD_COPY))) {
	 ...
	 * @param user_name
	 */
	private final void setUser_name(String user_name) {
		this.user_name = user_name;
	}
	/**
	 * Obtiene la clave antigüa
	 * @return
	 */
	 ...
	 * @param old_password
	 */
	private final void setOld_password(String old_password) {
	...
	private final void setNew_password(String new_password) {
		this.new_password = new_password;
	}

	private final String getNew_passwdCopy() {
		return new_passwdCopy;
	}

	private final void setNew_passwdCopy(String new_passwdCopy) {

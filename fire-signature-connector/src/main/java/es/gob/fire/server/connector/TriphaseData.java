...
public final class TriphaseData {
	...
	static final Logger LOGGER = Logger.getLogger(TriphaseData.class.getName());
	...
	public static final class TriSign {
		...
		/**
		 * Obtiene la informaci&oacute;n del documento.
		 * @return Objeto con los datos conocidos del documento.
		 */
		public DocInfo getDocInfo() {
			return DocInfo.extractDocInfo(this.dict);
		}

		/**
		 * Establece la informaci&oacute;on del documento.
		 * @param docInfo Informacion conocida del documento.
		 */
		public void setDocInfo(final DocInfo docInfo) {
			DocInfo.addDocInfoToSign(this.dict, docInfo);
		}

		/**
		 * Obtiene la prefirma de la firma trif&aacute;sica.
		 * @return Prefirma de la firma trif&aacute;sica.
		 */
		public byte[] getPresign() {
			byte[] pre = null;
			if (this.dict.containsKey(PROPERTY_NAME_PRESIGN)) {
				try {
					pre = Base64.decode(this.dict.get(PROPERTY_NAME_PRESIGN));
				} catch (final IOException e) {
					LOGGER.log(Level.WARNING, "No se puede decodificar una prefirma", e); //$NON-NLS-1$
					return null;
				}
			}
			return pre;

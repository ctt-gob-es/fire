...
public class ChooseCertificateOriginService extends HttpServlet {
	...
	protected void service(final HttpServletRequest request, final HttpServletResponse response) {
		...
		// Ya que este es uno de los puntos de entrada del usuario a la operativa de
		// FIRe, se establece aqui
		...
		final boolean originForced = Boolean
				.parseBoolean(request.getParameter(ServiceParams.HTTP_PARAM_CERT_ORIGIN_FORCED));
		...
		if (SessionFlags.OP_SIGN != session.getObject(ServiceParams.SESSION_PARAM_PREVIOUS_OPERATION)) {
			session = SessionCollector.getFireSessionOfuscated(transactionId, subjectRef, request.getSession(false),
					false, true, trAux);
		}
		...
		// Si llegamos hasta aqui usando correctamente el API, es que no se produjo
		// ningun
		// error al autenticar al usuario (si es que el conector requeria
		// autenticacion),
		...
		// Si no se selecciono firma local, se firmara con un proveedor de firma en la
		// nube
		else {
			final String subjectId = session.getString(ServiceParams.SESSION_PARAM_SUBJECT_ID);
			signWithProvider(origin, subjectId, transactionId, session, connConfig, request, response, originForced,
					trAux);
		}
	 ...
	 * @param originForced Indica si se forz&oacute; el uso de un proveedor
	 *                     concreto.
	 ...
	 * @param request      Petici&oacute;n HTTP realizada al servicio.
	 * @param response     Objeto HTTP de respuesta del servicio.
	 * @param errorUrl     URL a la que redirigir en caso de error hasta que se
	 *                     obtenga la de sesi&oacute;n.
	 * @param originForced Indica si se forz&oacute; el uso de un proveedor
	 *                     concreto.
	 * @param trAux        Informaci&oacute;n auxiliar de la transacci&oacute;n.
	 */
		...
		try {
			...
			if (certificates == null || certificates.length == 0) {
				if (connector.allowRequestNewCerts()) {
					LOGGER.info(
							logF.f("El usuario no dispone de certificados, pero el conector le permite generarlos")); //$NON-NLS-1$
			...
			LOGGER.log(Level.SEVERE,
					logF.f("No se ha podido cargar el conector del proveedor de firma: %1s", providerName), e); //$NON-NLS-1$

...
public class ResponseClave {
	 ...
	 */
    private static final Logger LOG = LoggerFactory.getLogger(ResponseClave.class);
    ...
    public String responseClave(final HttpServletRequest request, final HttpServletResponse response, final Model model) {
		final AtomicReference<String> dniRef =  new AtomicReference<>("");
		try {
			LOG.info(Language.getResWebAdminGeneral(IWebAdminGeneral.UD_LOG013));
	    	...
	    	final Properties spConfig = SPConfig.loadSPConfigs();

	    	final String claveReturnUrl = spConfig.getProperty(Constants.SP_RETURN);

	    	// Validaremos la respuesta devuelta por clave
	    	final IAuthenticationResponseNoMetadata authnResponse = validateRespAndActivateCertContigency(claveReturnUrl, samlResponse, relayState, remoteHost);

	    	// Eliminamos intentos fallidos de acceso para todas las ip
	    	this.iLoginService.deleteAllControlAccess();

	    	final PersonalInfoBean personalInfoBean = this.obtenerDatosUsuario(authnResponse);
...
//	    	response.addCookie(cookie);
	        ...
	        LOG.info(Language.getFormatResWebAdminGeneral(IWebAdminGeneral.UD_LOG007, new Object[] {user.getName(), user.getDni(), Language.getResWebAdminGeneral(IWebAdminGeneral.UD_LOG015)}));
			...
			LOG.info(Language.getFormatResWebAdminGeneral(IWebAdminGeneral.UD_LOG008, new Object[] {dniRef.get()}));
			model.addAttribute("errorMessage", e.getMessage());
			return "login.html";
		}catch (final Exception e) {
			LOG.error("Ocurrio un error al procesar la respuesta de Clave", e);
     ...
     */
    public PersonalInfoBean obtenerDatosUsuario(final IAuthenticationResponseNoMetadata authnResponse) {

		final ImmutableMap<AttributeDefinition<?>, ImmutableSet<? extends AttributeValue<?>>> attrMap
			= authnResponse.getAttributes().getAttributeMap();

		final String infoToken = SecureRandomXmlIdGenerator.INSTANCE.generateIdentifier(8);
     ...
     */
	private IAuthenticationResponseNoMetadata validateRespAndActivateCertContigency(final String claveReturnUrl,
			final String samlResponse, final String relayState, final String remoteHost) throws ClaveException {
		...
		final byte[] decSamlToken = EidasStringUtil.decodeBytesFromBase64(samlResponse);
		IAuthenticationResponseNoMetadata authnResponse = null;
		try {
			final ProtocolEngineNoMetadataI protocolEngine = SpProtocolEngineFactory.getSpProtocolEngine(Constants.SP_CONF, SPConfig.getConfigFilePath());
			...
			// Check session
			final String prevRelayState = SessionHolder.sessionsSAML.get(authnResponse.getInResponseToId());
			...
			LOG.error("Error al validar la respuesta de Cl@ve",e);
			throw new ClaveException(Language.getResWebAdminGeneral(IWebAdminGeneral.UD_LOG009));
		}
     ...
     */
    private static String extractFromAttrMap(final String friendlyName, final ImmutableMap<AttributeDefinition<?>, ImmutableSet<? extends AttributeValue<?>>> attrMap) {
		final Iterator<AttributeDefinition<?>> it = attrMap.keySet().iterator();
		while (it.hasNext()) {
			final AttributeDefinition<?> k = it.next();
			if (friendlyName.equals(k.getFriendlyName())) {
				final ImmutableSet<? extends AttributeValue<?>> valuesSet = attrMap.get(k);

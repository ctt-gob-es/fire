 ...
 */
package es.gob.fire.signature.connector.clavefirma;
...
import es.gob.fire.signature.connector.BadConfigurationException;
...
public final class ClaveFirmaConnector implements FIReConnector {
	...
	private static final String CONFIG_FILE = "gatewayapi.properties" ; //$NON-NLS-1$
    ...
    public void init(final Properties config) {
        ...
        this.providerName = ConfigManager.getClaveFirmaProviderName();
    }
	 ...
	 */
	private static GateWayAPI getGateWayApi() {

        final GateWayAPI gatewayApi = new GateWayAPI();

        try {
        	final Properties gatewayConfig = ConfigFileLoader.loadConfigFile(CONFIG_FILE);
        	gatewayApi.setConfig(gatewayConfig);
        }
        catch (final Exception e) {
        	LOGGER.warning("No se puede cargar el fichero '" + CONFIG_FILE + //$NON-NLS-1$
        			"' para la configuracion de la biblioteca de acceso a ClaveFirma. " + //$NON-NLS-1$
        			"Se usara la configuracion por defecto: " + e); //$NON-NLS-1$
		}
        return gatewayApi;
	}

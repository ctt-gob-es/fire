...
class StackTraceFormatter {
    ...
    private StackTraceFormatter(final StringBuilder builder, final int suppressedDepth, final boolean extended) {
        ...
        this.cache = extended ? new HashMap<String, String>() : null;
    }
    ...
    private void renderStackTrace(final Throwable t) {
        ...
        if (this.suppressedDepth != 0) {
            for (final Throwable se : t.getSuppressed()) {
                if (this.suppressedDepth < 0 || this.suppressedDepth > this.suppressedCount++) {
    ...
    private void renderStackTrace(final StackTraceElement[] parentStack, final Throwable child, final String caption, final String prefix) {
        if (this.seen.contains(child)) {
            this.builder.append("\t[CIRCULAR REFERENCE:")
                    .append(child)
                    .append(']');
            newLine();
        } else {
            ...
            if (this.suppressedDepth != 0) {
                for (final Throwable se : child.getSuppressed()) {
                    if (this.suppressedDepth < 0 || this.suppressedDepth > this.suppressedCount++) {
    ...
    private void renderExtended(final String prefix, final StackTraceElement element) {
        ...
        final String cached;
        if ((cached = this.cache.get(className)) != null) {
     ...
     */
    static String getJarName(final URL resource, final String classResourceName) {
        if (resource == null) {
            return null;
        }
        ...
        for (int endIdx = path.lastIndexOf(classResourceName); endIdx >= 0; endIdx--) {
            final char ch = path.charAt(endIdx);
            if (ch == '/' || ch == '\\' || ch == '?') {
